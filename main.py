...
if __name__ == '__main__':
    ...
    source_directory = settings.persist_directory+"init_dataset_small"

    basenameDataset = source_directory.split("/")[-1]
    print(basenameDataset)
    destination_directory = "keywords_suggester/data_transformed/"+basenameDataset+"_"+str(ts)
    ...
    # I CAN CREATE SINGLETON and then add documents
    
    vectordb.persist()  
    print("DONE CHROMA PERSIST")

    collection = vectordb.get()
    print("COLLECTION COUNT: "+str(len(collection.keys()))) #ids, metadata, documents